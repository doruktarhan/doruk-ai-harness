...
function createTypesafePolicy(credentials: JevCredentials): JevPolicy {
	let textModel: LanguageModel | undefined;
	return {
		async choose(observation, goal, history, signal) {
			const questions = buildQuestions(observation, goal);
		...
		async text(observation, goal, target, history, signal) {
			if (!credentials.gatewayApiKey) {
				throw new Error(
					`TYPE_TEXT still requires AI_GATEWAY_API_KEY in the process environment or gateway.apiKey in ${CONFIG_PATH}.`,
				);
			}
			textModel ??= createGateway({ apiKey: credentials.gatewayApiKey })(
				credentials.textModel,
			);
			return (
				await generateFieldText(
					textModel,
