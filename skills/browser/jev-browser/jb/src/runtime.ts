...
export class JevBrowserManager {
	...
	private async startBrowser(
		...
		const launchOptions = {
			...
			// Cline itself is running in an app/plugin subprocess. The full Chromium
		...
		try {
			...
			launchWarning =
				"Headless Chromium was unavailable in the Cline macOS process, so Jev Browser started a visible browser window.";
		}
...
function emitHostUpdate(type: string, payload: unknown) {
	(
		globalThis as typeof globalThis & { __clinePluginHost?: HostBridge }
	).__clinePluginHost?.emitEvent?.("jev_browser_update", { type, payload });
}
